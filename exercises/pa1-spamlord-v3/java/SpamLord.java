...
public class SpamLord {
  ...
  private Pattern emailPattern = 
      ...
      ("(-?\\w-?(?:(?:\\w|\\w-)\\.?)+)"+ //user
       ...
       " ?(-?\\w-?(?:(?:\\w|\\w-)[\\.|;| ]?){1,20})"+            //server
       " ?(?:\\.|;| |do?t|DOM)"+              //last dot
       " ?(-?e-?d-?u|-?E-?D-?U|-?c-?o-?m\\))");                     //suffix
  private Pattern obfuscatedEmailPattern=
      Pattern.compile(
        "obfuscate\\s*?\\(\\s*?"+
        "['\"](\\w(?:\\w|\\.)+)['\"]"+
        "\\s*?,\\s*?"+
  ...
  public List<Contact> processFile(String fileName, BufferedReader input) {
    ...
    try {
      for(String line = input.readLine(); line != null; line = input.readLine()) {
        ...
        o = obfuscatedEmailPattern.matcher(line);
        while(m.find()) {
            String user=m.group(1).toLowerCase().trim();
            String server=m.group(2);
            String suffix=m.group(3).toLowerCase().trim().replaceAll("\\W","");
            if(!"server".equals(user)) {
                email = user + "@" + server + "." + suffix;
                String formatted=
                    email.replaceAll("-","").
                    replaceAll(" |;|do?t|DOM",".").
                    replaceAll("\\.+",".");
                contact = new Contact(fileName,"e",formatted);
                contacts.add(contact);
            }
        }
        while (o.find()) {
            String user=o.group(2);
            String server=o.group(1);
            email = user + "@" + server;
            contact = new Contact(fileName,"e",email);
            contacts.add(contact);
        }
        t = telephonePattern.matcher(line);
        while (t.find()) {
            tel= t.group(1)+"-"+t.group(2)+"-"+t.group(3);
            contact= new Contact(fileName,"p",tel);
            contacts.add(contact);
        }
      }
      input.close();
